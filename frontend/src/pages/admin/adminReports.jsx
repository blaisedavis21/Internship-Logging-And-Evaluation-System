...
import { mockLogs, mockPlacements, mockEvaluations, mockUsers } from "@/data/mockData";
...
const AdminReports = () => {
  const hoursData = mockUsers
    ...
    .map((u) => ({
      name: u.name.split(" ")[0],
      hours: mockLogs.filter((l) => l.studentId === u.id).reduce((sum, l) => sum + l.hoursWorked, 0),
    }))
    .filter((d) => d.hours > 0);

  const placementStatus = [
    { name: "Active", value: mockPlacements.filter((p) => p.status === "active").length, color: "hsl(160 84% 39%)" },
    { name: "Completed", value: mockPlacements.filter((p) => p.status === "completed").length, color: "hsl(220 15% 55%)" },
    { name: "Pending", value: mockPlacements.filter((p) => p.status === "pending").length, color: "hsl(40 80% 50%)" },
  ];

  const weeklyTrend = Array.from({ length: 4 }, (_, i) => ({
    week: `Week ${i + 1}`,
    logs: mockLogs.filter((l) => l.weekNumber === i + 1).length,
    hours: mockLogs.filter((l) => l.weekNumber === i + 1).reduce((s, l) => s + l.hoursWorked, 0),
  }));
      ...
      <div className="max-w-7xl mx-auto">
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="mb-8">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[11px] font-bold text-accent uppercase tracking-[0.15em]">Reports</span>
          </div>
          <h1 className="text-3xl font-bold font-display text-foreground tracking-tight">Reports & Analytics</h1>
            ...
            <div className="flex-1 min-h-[240px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={hoursData}>
                  ...
                  <Bar dataKey="hours" fill="hsl(45 93% 58%)" radius={[6, 6, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </motion.div>
            ...
            <div className="flex-1 min-h-[260px]">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={weeklyTrend} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                  ...
                  <YAxis tick={{ fontSize: 11, fill: "hsl(220 15% 55%)" }} axisLine={false} tickLine={false} />
                  <Tooltip {...tooltipStyle} />
                  <Area type="monotone" dataKey="logs" stroke="hsl(45 93% 58%)" strokeWidth={2.5} fill="url(#logsGrad)" dot={{ r: 4, fill: "hsl(45 93% 58%)", strokeWidth: 0 }} name="Log Entries" />
                  <Area type="monotone" dataKey="hours" stroke="hsl(217 91% 60%)" strokeWidth={2.5} fill="url(#hoursGrad)" dot={{ r: 4, fill: "hsl(217 91% 60%)", strokeWidth: 0 }} name="Hours" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </motion.div>
        ...
        {/* Summary */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="card-premium p-6 sm:p-8 overflow-hidden relative">
          ...
          <div className="relative z-10">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
              {[
                ...
                >
                  <p className="text-4xl font-bold text-gradient-gold font-display">{item.value}</p>
                  <p className="text-[11px] text-muted-foreground mt-1 font-bold uppercase tracking-wider">{item.label}</p>
                </motion.div>
              ))}
            </div>
          </div>
